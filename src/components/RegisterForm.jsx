...
export default function RegisterForm() {
  ...
  try {
    ...
    // ✅ Only generate ticket after DB save success
    if (data.success) {
      setTicketData({ ...form, _id: data.userId });
    } else {
  ...
  <label
    htmlFor="name"
    className="flex items-center gap-2 text-sky-300 mb-2"
  >
...
/>
  ...
  <GraduationCap size={16} /> Year
              ...
              {/* Events */}
              <div>
               <p className="text-sky-300 mb-4 block">
  Select Events
</p>
  ...
  {form.paymentStatus === "paid" && (
    <>
      ...
      <AnimatePresence>
        {ticketData && (
          <Ticket data={ticketData} onClose={() => setTicketData(null)} />
        )}
      </AnimatePresence>
    </>
  );
